...
const reponses = ['b', 'c', 'c', 'b', 'b'];
const emojis = ['✔️','✨','👀','😭','👎'];
const titreResultat = document.querySelector(".resultats h2"); 
const noteResultat = document.querySelector(".note");
const aideResultat = document.querySelector(".aide");
const toutesLesQuestions = document.querySelector(".question-block");
let veryArray = [];
...
form.addEventListener('submit', (e) => {
   ...
   verifyFunction(tableauResultats);
   tableauResultats = [];
})


function verifyFunction(tabResultats){
    ...
    for (let a = 0; a < 5; a++){
        ...
        if (tableauResultats[a] === reponses[a]){
            ...
            veryArray.push(false);
        }
    }

    afficherResultats(veryArray);
    veryArray = [];
}
